...
public class PollsDAO implements IPollsDAO{
	...
	public String createPoll(JSONObject jsonCreatePollValues) throws Exception{ 
		...
		try{
			...
			if(null==jsonCreatePollValues.get("poll_question") || null==jsonCreatePollValues.get("poll_create_date")|| 
					null==jsonCreatePollValues.get("poll_end_date") || null==jsonCreatePollValues.get("poll_creator")||
					null==jsonCreatePollValues.get("poll_category") || null==jsonCreatePollValues.get("poll_public") ||
					jsonCreatePollValues.get("poll_question").toString().isEmpty() || 
					jsonCreatePollValues.get("poll_create_date").toString().isEmpty() || 
					jsonCreatePollValues.get("poll_end_date").toString().isEmpty() ||
					jsonCreatePollValues.get("poll_creator").toString().isEmpty() ||
					jsonCreatePollValues.get("poll_category").toString().isEmpty() || 
					jsonCreatePollValues.get("poll_public").toString().isEmpty()){
				result.put("Msg", "fail");
			}
			else{
				String flag="success";
	...
	public String deletePoll(String jsonDeletePollId) throws Exception{ 
		...
		try{
			...
			if(null==jsonDeletePollId || jsonDeletePollId.isEmpty()){
				result.put("Msg","fail");
			}
			else{
			...
			if(null==user_name || user_name.isEmpty()){
				...
				try{
					...
					if(auth){
						...
						whereQuery.put("poll_end_date", new BasicDBObject("$gte",dateFormat.format(cal.getTime())));
...
//					cursorResult.put("poll_participants",getdata.get("poll_participants"));
				...
				try{
					...
					if(auth){
						...
						for(int i=0;i<cursorResult.length();i++){
							...
							whereQuery1.put("poll_end_date", new BasicDBObject("$gte",dateFormat.format(cal.getTime())));
	...
	public String showPollsByCategory(String categoryName,String user_name) throws Exception{
		...
		try{
			//removing extra chars from the  input --> validation
			if(null==categoryName || null==user_name || categoryName.isEmpty() || user_name.isEmpty()){
				result.put("Msg", "fail");
			}
			else{
				user_name=user_name.replaceAll("[^0-9]", "").trim();
				...
				try{
					...
					if(auth){
						...
						whereQuery.put("poll_creator", new BasicDBObject("$ne", user_name));
						whereQuery.put("poll_public", "yes");
						DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy");
						Calendar cal = Calendar.getInstance();
						whereQuery.put("poll_end_date", new BasicDBObject("$gte",dateFormat.format(cal.getTime())));
	...
	public String voteOnPoll(JSONObject pollResults) throws Exception{
		...
		try{
			//removing extra chars from the  input --> validation
			if(null==pollResults.get("poll_id") || null==pollResults.get("poll_question") ||
					null==pollResults.get("poll_voter_option") || null==pollResults.get("poll_voter_id")	||
					null==pollResults.get("poll_creator") || null==pollResults.get("poll_category") ||
					null==pollResults.get("poll_create_date") || null==pollResults.get("poll_end_date") ||
					pollResults.get("poll_id").toString().isEmpty() || pollResults.get("poll_question").toString().isEmpty() ||
					pollResults.get("poll_voter_option").toString().isEmpty() || pollResults.get("poll_voter_id").toString().isEmpty()	||
					pollResults.get("poll_creator").toString().isEmpty() || pollResults.get("poll_category").toString().isEmpty() ||
					pollResults.get("poll_create_date").toString().isEmpty() || pollResults.get("poll_end_date").toString().isEmpty()){
				result.put("Msg", "fail");
			}else{
